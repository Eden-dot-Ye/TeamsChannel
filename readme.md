...
1. 通过V0生成项目骨架，并创建Supabase数据库。（利用个人账号免费额度）
2. 通过VSCode中的Copilot Agent功能，快速修改UI界面设计和功能开发。（利用公司Github Enterprise订阅）
3. 使用Azure中部署的OpenAI模型，实现AI自动分类功能。（利用公司Azure订阅）
4. 利用Vercel部署这个Demo项目，方便访问和展示。（使用免费的部署服务）
   1. 也尝试过通过Azure的APP Service部署这个Demo项目，但是它对NextJS的支持不是很好，导致部署后页面打不开也没找到解决方法，所以最后选择了Vercel来部署。
5. 使用自己的域名解析到Vercel部署的项目上，避免Vercel默认域名在公司电脑上打不开的问题。（使用自己购买的域名）

## 项目耗时

1. Idea构思和初步Prompt：1小时
2. V0初步项目骨架搭建：1小时
3. 本地Agent微调：30分钟
4. 整合Azure OpenAI模型：30分钟
5. 部署到Vercel：2小时（主要时间在调试部署配置上）
6. 用自己的域名解析到Vercel部署的项目上：30分钟
7. Logo设计：10分钟

## 经验

1. 使用Vibe Coding时，一定要看聊天记录
   1. 否则无法提高自己的能力
   2. 并且如果模型没解决问题，也不该立刻换模型，而是应该先分析聊天记录，看看问题出在哪里，是模型能力不足，还是Prompt设计不合理（没有给到问题根源的提示）
2. 一个问题：如何让模型回复中文
...
之前的问题解决的不错，但是有如下仍可以调整的地方
1. 如果发帖的时候，用户发的内容是一个无意义的问题，那么目前会输出：Failed to categorize message，但我希望它需要必须分配给一个已经存在的频道
